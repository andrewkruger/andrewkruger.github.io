...
Now for the structure of the neural network!  The following is a 2-layer Neural Network.  It has an input layer (not included in the layer count), one hidden layer, and an output layer.  Increasing the number of layers allows for more complex models (analogous to polynomial fits compared to linear fits).

Pooling is done by looking at a grid (here a 2x2 grid) and only using the most significant value.  If no 
...
Create the input layer.  The `input_shape` is 28x28, the shape of the images, and the activation function is *ReLU*.  The second layer is of 
