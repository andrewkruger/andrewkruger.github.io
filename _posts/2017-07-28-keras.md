...
A binary class matrix is boolean, either False or True, determined by the activation function.  Here the neurons are set to be binary.
...
Now for the structure of the neural network!  The following is a 2-layer Neural Network with one hidden layer.  It has an input layer (not included in the layer count), one hidden layer, and an output layer.  Increasing the number of layers allows for more complex models (analogous to polynomial fits compared to linear fits).

The Neural Network is first set to be sequential (and every layer after is inferred to be sequantial).
...
Create the input layer.  The `input_shape` is 28x28, the shape of the images, and the activation function is *ReLU*.

A [dropout](http://www.cs.toronto.edu/~rsalakhu/papers/srivastava14a.pdf) is used to prevent overfitting by setting any fractional input rates at or below the dropout value to zero.  This will remove neurons in the network, leaving behind neural connections with heavier weights.
